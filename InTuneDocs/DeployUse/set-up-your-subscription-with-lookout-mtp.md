...
För att göra din prenumeration redo för Lookout MTP-tjänsten behöver Lookout-supporten (enterprisesupport@lookout.com) följande information om Azure Active Directory (AD Azure). 
...
Använd informationen i följande avsnitt för att samla in den information du måste förse Lookout-supportteamet med.  

## Hämta din Azure AD-information
### Hämta din Azure AD-klient-ID
Logga in på Azure AD-hanteringsportalen https://manage.windowsazure.com och välj din prenumeration. 

![skärmbild av Azure AD-sidan som visar namnet på klienten](../media/mtp/aad_tenant_name.png) När du väljer namnet på din prenumeration kommer URL:en som skapas att innehålla ditt prenumerations-ID.  Om du har problem med att hitta ditt prenumerations-ID kan [Microsoft support article](https://support.office.com/en-us/article/Find-your-Office-365-tenant-ID-6891b561-a52d-4ade-9f39-b492285e2c9b?ui=en-US&rs=en-US&ad=US) (Microsofts support-artikel) ge ytterligare tips.   
...
* **Begränsad åtkomst:** Ingen åtkomst till ett flertal konfigurations- och registreringsrelaterade moduler i Lookout MTP-konsolen, skrivskyddad åtkomst till **Säkerhetsprincip**-modulen i Lookout MTP-konsolen.  
...
**Group Object ID** (Grupp-objekt-ID:t) finns på egenskapssidan för gruppen i Azure AD-hanteringskonsolen.
...
Nästa steg beskriver de uppgifter som måste utföras för att slutföra konfigurationen av Lookout MTP i [Lookout MTP Console](https://aad.lookout.com) (Lookout MTP-konsolen).

### Steg 2: Konfigurera Intune Connector

I Lookout MTP-konsolen går du till **System**-modulen, väljer **Kopplingar** och sedan **Intune**.
...
Konfigurera pulsslagsfrekvensen i minuter i alternativen för anslutningsinställningar.  I och med slutförande av det här steget är Intune-anslutningen är nu klar.  
...
För alternativet **Enrollment Management** (Registreringshantering), ange ett antal användare vars enheter ska registreras med Lookout.   Det är rekommenderat att börja med en liten grupp användare för att testa och bekanta dig med hur integrationen fungerar.  När du är nöjd med testresultaten kan du utöka registreringen till grupper av användare.

Ange först en Azure AD-säkerhetsgrupp som du tycker skulle vara en bra grupp användare att registrera i Lookout MTP för att komma igång med registreringsgrupper. När gruppen har skapats i Azure AD går du till alternativet **Enrollement Management** (Registreringshantering) i Lookout MTP-konsolen och lägger till Azure AD-säkerhetsgruppen **Visningsnamn** för registrering.

När en användare finns i en grupp för registrering registreras alla dennes enheter som identifieras och har stöd i Azure AD, och dessa enheter kan aktiveras i Lookout MTP.  Första gången de öppnar Lookout for Work-appen på en enhet som stöds, aktiveras den i Lookout MTP.
![skärmbild som visar sidan registrering av Intune-anslutning](../media/mtp/lookout-mtp-enrollment.png)

Det rekommenderas att lämna steget att söka efter nya enheter på de fem minuter som är standard.
...
* Det finns ingen validering av visningsnamn för grupper.  Se till att värdet i fältet **VISNINGSNAMN** som visas i Azure AD-säkerhetsgruppen i Azure-portalen.
...
För alternativet **State Sync** (Programtillståndssynkronisering) anger du vilken typ av data som ska skickas till Intune.  Du måste för närvarande aktivera både enhetsstatus och hotstatus för att Intune-Lookout-integrationen ska fungera korrekt.  De här verktygen är aktiverade som standard.
...
Om du vill ta emot e-postaviseringar för hot, logga in i [Lookout MTP-konsolen](https://aad.lookout.com) med det användarkonto som ska ta emot meddelanden.  Gå till **System**-modulen och under fliken **Inställningar** väljer du den önskade typen av meddelanden och sätter dem på **PÅ**. Spara ändringarna.
...
När installationen är klar börjar Lookout MTP avsöka Azure AD efter enheter som motsvarar de angivna registreringsgrupperna.  Information om de enheter som registrerats finns i modulen Enheter.  Den första statusen för enheterna är ”väntar”.  Enhetens status kommer att ändras när Lookout for work-appen är installerad, öppnas och aktiveras på enheten.  Information om hur du skickar Lookout for work-appen till enheten finner du i artikeln [configure and deploy Lookout for work apps](configure-and-deploy-lookout-for-work-apps.md) (konfigurera och distribuera Lookout for work-appar).
## Nästa steg
