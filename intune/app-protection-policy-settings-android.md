...
| Inställningen | Använd så här | Standardvärde |
|------|------|------|
| **PIN-kod för åtkomst** | Välj **Kräv** om en PIN-kod ska krävas för att använda den här appen. Användarna uppmanas att konfigurera denna PIN-kod första gången de kör appen i en arbets- eller skolkontext. <br><br> Konfigurera följande inställningar för PIN-styrka:| **Kräv** |
|<ul><ui> **Typ av PIN-kod** | Ange ett krav på PIN-kod av numerisk typ eller lösenordstyp innan en app med appskyddsprinciper används. Numeriska krav har endast siffror, medan ett lösenord kan definieras med minst en alfabetisk bokstav **eller** minst ett specialtecken. <br><br>**Obs:** *Specialtecken tillåts, däribland specialtecken och symboler på det engelska Android-tangentbordet.*| **Numerisk** |
|<ul><ui> **Enkel PIN-kod** |Välj **Tillåt** om du vill tillåta att användarna använder enkla PIN-kodsekvenser, till exempel *1234*, *1111*, *abcd* eller *aaaa*. Välj **Blockera** om du vill förhindra att användarna använder enkla sekvenser. <br><br>**Obs:** *Om Typ av PIN-kod har värdet Lösenord och Enkel PIN-kod har värdet Tillåt måste användarna använda minst en bokstav **eller** minst ett specialtecken i PIN-koden. Om Typ av PIN-kod har värdet Lösenord och Enkel PIN-kod har värdet Blockera måste användarna använda minst ett tal **och** en bokstav **och** minst ett specialtecken i PIN-koden.* | **Tillåt** |
| <ul><ui> **Välj minimilängd för PIN-kod** | Ange det minsta antalet siffror i en PIN-kodsekvens.  | **4** |
|<ul><ui> **Fingeravtryck istället för PIN-kod för åtkomst (Android 6.0+)** | Välj **Tillåt** om du vill tillåta att [autentisering med fingeravtryck](https://developer.android.com/about/versions/marshmallow/android-6.0.html#fingerprint-authentication) används i stället för en PIN-kod vid åtkomst till appen. <br><br>**Obs:** *Den här funktionen stöder allmänna biometrikontroller på Android-enheter. OEM-specifika biometriska inställningar, t.ex. *Samsung Pass*, stöds inte.* <br><br>På Android kan du låta användaren bekräfta sin identitet med hjälp av [Android-autentisering med fingeravtryck](https://developer.android.com/about/versions/marshmallow/android-6.0.html#fingerprint-authentication) i stället för en PIN-kod. När användarna försöker använda appen med ett arbets- eller skolkonto uppmanas de att lämna sitt fingeravtryck i stället för att ange en PIN-kod. <br><br> Android-arbetsprofiler kräver att ett separat fingeravtryck registreras för att principen **Tillåt fingeravtryck i stället för PIN-kod** ska tillämpas. Den här principen tillämpas endast för principhanterade appar som har installerats i Android-arbetsprofilen. Det separata fingeravtrycket måste registreras med enheten efter att den hanterade Android-arbetsprofilen har skapats genom registrering på företagsportalen. Mer information om fingeravtryck för arbetsprofiler med Android-arbetsprofiler finns i [Lås din arbetsprofil](https://support.google.com/work/android/answer/7029958). |**Tillåt** |
| <ul><ul><ui>**Åsidosätt fingeravtryck med PIN-kod efter tidsgräns** |  När det är inställt på **Kräv** åsidosätter en PIN-prompt Touch ID-prompter när appen har varit inaktiv under den *tidsgränsperiod* du anger.   | **Kräv** |
| <ul><ul><ui> **Tidsgräns (minuters inaktivitet)** | Ange i minuter hur länge appen kan vara inaktiv innan Touch ID-prompterna ersätts av PIN-prompter.  <br><br>Det här tidsgränsvärdet ska vara större en värdet som anges under *Kontrollera åtkomstbehörigheterna på nytt efter (minuters inaktivitet)*.| **30** |
|<ul><ui> **Återställ PIN-kod efter antal dagar** | Välj **Ja** för att kräva att användarna ändrar PIN-kod för appen efter en viss tid, i dagar. <br><br> Standardvärde = **Nej** <br><br> När värdet är *Ja* kan du sedan konfigurera antalet dagar innan återställning krävs. | **Nej**|
| <ul><ul><ui>**Antal dagar** | När *Återställ PIN-kod efter antal dagar* är inställt på Ja anger du hur lång tid innan som återställningen av PIN-koden krävs. | **0** | 
|<ul><ui> **PIN-appkod när enhetens PIN-kod har ställts in** | Om det är inställt på **Inaktivera** behövs ingen app-PIN-kod på en MDM-registrerad enhet som har en enhets-PIN-kod inställd.   | **Aktivera** |
...
| **Kontrollera åtkomstbehörigheterna på nytt efter (minuters inaktivitet)** | Konfigurera antalet minuter med inaktivitet som måste passera innan appen kräver att användaren anger uppgifter för åtkomst på nytt.  <br><br>**Obs:** *På Android delas PIN-koden med alla Intune-hanterade appar. PIN-timern återställs när appen lämnar enhetens förgrund. Användaren behöver inte ange en PIN-kod på någon Intune-hanterad app som delar sin PIN under perioden för den tidsgräns som anges i denna inställning.*| **30** |
